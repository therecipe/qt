...
{
	Q_UNUSED(ptr);
	return new QJSValue(({QJSValue tmp = static_cast<QList<QJSValue>*>(p)->at(i); if (i == static_cast<QList<QJSValue>*>(p)->size()-1) { static_cast<QList<QJSValue>*>(p)->~QList(); free(p); }; tmp; }));
}
