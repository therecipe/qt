...
RUN find /opt/Qt5.6.3/gcc_64 -type f ! -name "*.a" ! -name "*.la" ! -name "*.prl" -name "lib*" -exec strip -s {} \;
...
ENV QT_DIR /opt/Qt5.6.3
ENV QT_DOCKER true
ENV QT_VERSION 5.6.3
...
COPY --from=base /opt/Qt5.6.3/5.6.3/gcc_64 /opt/Qt5.6.3/5.6.3/gcc_64
