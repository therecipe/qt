...
RUN OSSL=openssl-1.0.2r.tar.gz && curl -sL --retry 10 --retry-delay 60 -O https://www.openssl.org/source/$OSSL && tar -xzf $OSSL -C $HOME
...
RUN sed -i 's/LIBCOMPATVERSIONS=/#LIBCOMPATVERSIONS=/g' $HOME/openssl-1.0.2r/Makefile
RUN sed -i 's/LIBVERSION=/\\ #LIBVERSION=/g' $HOME/openssl-1.0.2r/Makefile
RUN cd $HOME/openssl-1.0.2r && ./Configure shared android-armv7 && make -j $(grep -c ^processor /proc/cpuinfo) build_libs $HOME/openssl-1.0.2r
RUN mkdir -p $HOME/openssl-1.0.2r-arm && mv $HOME/openssl-1.0.2r/*.so* $HOME/openssl-1.0.2r-arm

RUN rm -rf $HOME/openssl-1.0.2r && tar -xzf openssl-1.0.2r.tar.gz -C $HOME
...
RUN sed -i 's/LIBCOMPATVERSIONS=/#LIBCOMPATVERSIONS=/g' $HOME/openssl-1.0.2r/Makefile
RUN sed -i 's/LIBVERSION=/\\ #LIBVERSION=/g' $HOME/openssl-1.0.2r/Makefile
RUN cd $HOME/openssl-1.0.2r && ./Configure shared android-x86 && make -j $(grep -c ^processor /proc/cpuinfo) build_libs $HOME/openssl-1.0.2r
RUN mkdir -p $HOME/openssl-1.0.2r-x86 && mv $HOME/openssl-1.0.2r/*.so* $HOME/openssl-1.0.2r-x86
...
#RUN cd $HOME/openssl-1.1.1a && ./Configure -D__ANDROID_API__=16 android-arm && make -j $(grep -c ^processor /proc/cpuinfo) SHLIB_VERSION_NUMBER= SHLIB_EXT=.so build_libs $HOME/openssl-1.1.1a
...
#RUN cd $HOME/openssl-1.1.1a && ./Configure -D__ANDROID_API__=16 android-x86 && make -j $(grep -c ^processor /proc/cpuinfo) SHLIB_VERSION_NUMBER= SHLIB_EXT=.so build_libs $HOME/openssl-1.1.1a
...
COPY --from=therecipe/qt:linux /opt/Qt/5.12.4/android_armv7 /opt/Qt/5.12.4/android_armv7
COPY --from=therecipe/qt:linux /opt/Qt/5.12.4/android_x86 /opt/Qt/5.12.4/android_x86
COPY --from=therecipe/qt:linux /opt/Qt/5.12.4/android_arm64_v8a /opt/Qt/5.12.4/android_arm64_v8a
...
COPY --from=base $HOME/openssl-1.0.2r-arm $HOME/openssl-1.0.2r-arm
COPY --from=base $HOME/openssl-1.0.2r-x86 $HOME/openssl-1.0.2r-x86
