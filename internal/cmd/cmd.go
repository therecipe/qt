...
func ParseFlags() bool {
	...
	if api := utils.QT_API(""); api != "" && err == nil && !utils.QT_DOCKER() && !forDocker {
		if utils.GoListOptional("{{.Dir}}", "github.com/StarAurryon/qt/internal/binding/files/docs/"+api, "-find", "get doc dir") == "" {
			...
			if !utils.UseGOMOD("") {
				if o := utils.GoListOptional("{{join .Imports \"|\"}}", "github.com/StarAurryon/qt/internal/binding/files/docs", "get doc dir"); o != "" {
					for _, v := range strings.Split(o, "|") {
						fmt.Println(strings.TrimPrefix(strings.TrimSpace(strings.Replace(v, "'", "", -1)), "github.com/StarAurryon/qt/internal/binding/files/docs/"))
					}
				}
			} else {
...
func virtual(arg []string, target, path string, writeCacheToHost bool, docker bool, system string) {
	...
	if utils.UseGOMOD(path) {
		...
		if writeCacheToHost {
			...
			if strings.Contains(path, "github.com/StarAurryon/qt/internal/examples") && !strings.Contains(path, "github.com/StarAurryon/qt/internal/examples/androidextras") {
				gpath += pathseperator + gpfs
			} else {
