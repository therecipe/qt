...
import (
	...
	"github.com/akiyosi/qt/internal/cmd/deploy"
	"github.com/akiyosi/qt/internal/cmd/minimal"
	"github.com/akiyosi/qt/internal/cmd/moc"
	"github.com/akiyosi/qt/internal/cmd/rcc"
...
func Test(target string, docker, vagrant bool, vagrantsystem string) {
	...
	for cat, list := range examples {
		for _, example := range list {
			...
			path := filepath.Join(strings.TrimSpace(utils.GoListOptional("{{.Dir}}", "github.com/akiyosi/qt/internal/examples", "-find", "get doc dir")), example)
