...
func main() {
	...
	if pwd, _ := os.Getwd(); strings.Contains(pwd, "/deploy/") || core.QSysInfo_ProductType() == "ios" || core.QSysInfo_ProductType() == "android" {
		felgoApp.SetMainQmlFileName("qml/ShowcaseMain.qml") //to make qtdeploy runs work
	} else {
		felgoApp.SetMainQmlFileName(pwd + "/qml/ShowcaseMain.qml") //to make go run/build runs work
	}

	// use this instead of the above call to avoid deployment of the qml files and compile them into the binary with qt's resource system qrc
	// this is the preferred deployment option for publishing games to the app stores, because then your qml files and js files are protected
	// felgoApp.SetMainQmlFileName("qrc:/qml/ShowcaseMain.qml")
