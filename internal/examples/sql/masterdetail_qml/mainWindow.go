...
func (w *MainWindow) initWith(artistTable string, albumTable string, albumDetails *core.QFile, parent *widgets.QWidget) {
	...
	w.model = NewSortFilterModel(NewViewModel())
...
func (w *MainWindow) changeArtist(row int) {
	artist := w.artistView.Model().Index(row, 0, core.NewQModelIndex()).Data(int(core.Qt__DisplayRole)).ToString()
...
func (w *MainWindow) showArtistProfile(index *core.QModelIndex) {
	...
	name := w.model.Index(index.Row(), 2, core.NewQModelIndex()).Data(int(core.Qt__DisplayRole)).ToString()
	count := getAlbumCountForArtist(name)
...
func (w *MainWindow) showAlbumDetails(index *core.QModelIndex) {
	...
	artist := w.model.Index(index.Row(), 2, core.NewQModelIndex()).Data(int(core.Qt__DisplayRole)).ToString()
	title := w.model.Index(index.Row(), 1, core.NewQModelIndex()).Data(int(core.Qt__DisplayRole)).ToString()
	year := w.model.Index(index.Row(), 3, core.NewQModelIndex()).Data(int(core.Qt__DisplayRole)).ToString()
	albumId := w.model.Index(index.Row(), 0, core.NewQModelIndex()).Data(int(core.Qt__DisplayRole)).ToString()
...
func (w *MainWindow) createArtistGroupBox() *widgets.QGroupBox {
	...
	w.artistView.SetModel(NewListModel())
...
func (w *MainWindow) indexOfArtist(artist string) *core.QModelIndex {
	...
	for i := 0; i < w.model.RowCount(core.NewQModelIndex()); i++ {
		index := w.model.Index(i, 2, core.NewQModelIndex())
		if index.Data(int(core.Qt__DisplayRole)).ToString() == artist {
			return index
		}
	}
