...
func main() {
	...
	for _, example := range []string{path.Join("widgets", "line_edits"), path.Join("widgets", "video_player"), path.Join("quick", "calc"), path.Join("quick", "dialog"), path.Join("quick", "view"), path.Join("qml", "application"), path.Join("qml", "prop")} {
		var before = time.Now()
